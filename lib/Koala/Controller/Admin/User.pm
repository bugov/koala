...
sub list {
  ...
  $self->render('user/admin/list',
    user_list => $user_list, user_count => $user_count);
}
...
sub edit {
  my $self = shift;
  my $user = Koala::Model::User->new(id => int $self->param('id'))->load();
  $user->$_($self->param($_)) for qw/username email info role createAt/;
  $user->newPassword($self->param('password')) if $self->param('password');
  $user->save();
